...
def _tavily_search(question: str) -> str:
    ...
    try:
        resp = requests.post(
            ...
            json={"query": query, "search_depth": "advanced", "max_results": 6, "include_answer": True, "include_raw_content": False},
            timeout=15,
        )
...
BULL_PROMPT = """\
You are THE BULL — a sports betting analyst whose only job is to build the strongest possible case for YES resolving.
MARKET QUESTION: "{question}"
Current YES price: {yes_pct}%
LIVE WEB SEARCH RESULTS:
{web_context}
TASK: List every statistical, situational, and momentum-based reason why YES is likely to happen. Be specific — cite teams, scores, injury reports, odds from the sources. Do NOT hedge or mention reasons for NO.
...
BEAR_PROMPT = """\
You are THE BEAR — a sports betting analyst whose only job is to build the strongest possible case for NO resolving.
MARKET QUESTION: "{question}"
Current YES price: {yes_pct}%
LIVE WEB SEARCH RESULTS:
{web_context}
TASK: List every statistical, situational, and momentum-based reason why NO is likely to happen. Be specific. Do NOT hedge or mention reasons for YES.
Respond with ONLY a JSON object — no markdown fences:
{{
...
SINGLE_AGENT_PROMPT = """\
You are an expert sports betting analyst. Use the data below to determine whether this prediction market is mispriced.
MARKET QUESTION: "{question}"
...
NO_DATA_PROMPT = """\
You are an expert sports betting analyst. No live web data was available, so use your training knowledge.
MARKET QUESTION: "{question}"
Current YES price: {yes_pct}%  |  Current NO price: {no_pct}%
Give your best estimate of fair value based on historical base rates and general knowledge. Note in reasoning that this uses training data only.
RESPOND with ONLY a valid JSON object — no markdown fences:
{{
  "fair_value": <float 0.0-1.0>,
  "confidence": "low",
        ...
        try:
            ...
            if resp.status_code in [503, 429]:
                if attempt < max_retries - 1:
                    ...
                    return {"error": f"Gemini HTTP {resp.status_code}: Service Overloaded."}
...
async def _multi_agent_debate(question: str, yes_price: float, web_context: str) -> dict:
    ...
    if "error" in bull_raw or "error" in bear_raw:
        fallback_prompt = (SINGLE_AGENT_PROMPT if web_context else NO_DATA_PROMPT).format(question=question, yes_pct=yes_pct, no_pct=no_pct, web_context=web_context)
        await asyncio.sleep(2)
    ...
    judge_prompt = JUDGE_PROMPT.format(question=question, bull_case=bull_raw.get("bull_case", ""), bear_case=bear_raw.get("bear_case", ""))

    await asyncio.sleep(2)
    judge_raw = await _call_gemini_async(judge_prompt, max_tokens=4096)
    if "error" in judge_raw: return judge_raw

    sb_prob = parse_odds_to_prob(judge_raw.get("extracted_sportsbook_odds"))
    if sb_prob is not None:
        anchor_price, confidence = sb_prob * 0.95, "high"
    else:
        anchor_price, confidence = yes_price, "medium"
    ...
    fair_value = max(0.01, min(0.99, anchor_price + qualitative_shift))
    edge_pct = fair_value - yes_price
    
    if edge_pct >= 0.02: verdict = "BUY_YES"
    elif edge_pct <= -0.02: verdict = "BUY_NO"
    else: verdict = "FAIR"

    return {
        "fair_value": float(fair_value), "confidence": confidence, "verdict": verdict, "edge_pct": float(edge_pct * 100),
        "reasoning": str(judge_raw.get("reasoning", "")), "key_facts": list(judge_raw.get("key_facts", [])),
        "sportsbook_implied": sb_prob, "debate_mode": True, "bull_summary": str(bull_raw.get("bull_case", "")[:200]),
        "bear_summary": str(bear_raw.get("bear_case", "")[:200]), "bull_prob": bull_prob, "bear_prob": bear_prob,
    }
...
async def call_gemini(question: str, yes_price: float) -> dict:
    ...
    try: result = await _multi_agent_debate(question, yes_price, web_context)
    except Exception as e: result = {"error": f"Debate pipeline failed: {e}"}
    return result
